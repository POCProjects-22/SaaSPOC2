...
{
    public class GenericRepository<T, TC> : IGenericRepository<T, TC>, IDisposable where T : class where TC : DbContext ,new()
    {
        ...
        public virtual List<T> Where(Expression<Func<T, bool>> expression)
        {
            ...
            var props = typeof(T).GetProperties();
            //if T doesn't inherit the base class, so check isdeleted column exists, or not
            if (props.Any(p => p.Name == "IsDeleted"))
            {
                // where IsDeleted=0
                var expIsDeleted = RepositoryExtensions.EqualExpression<T, bool>("IsDeleted", false);
                list = Entities.Where(expIsDeleted);                
            }
