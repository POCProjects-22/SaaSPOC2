...
{
    public class IdentityRepository<T, TC> : IIdentityRepository<T, TC>, IDisposable where T : class where TC : IdentityDbContext ,new()
    {

        private readonly IIdentityUnitOfWork<TC> unitOfWork;
        private DbSet<T> _entities;
        private string _errorMessage = string.Empty;
        private bool _isDisposed;
        public IdentityRepository(IIdentityUnitOfWork<TC> unitOfWork)
        : this(unitOfWork.Context)
        {
            this.unitOfWork = unitOfWork;
        }
        public IdentityRepository(TC context)
        {
            _isDisposed = false;
            Context = context;
        }
        ...
        public virtual IEnumerable<T> GetAll(bool ignoreIsDeleted = false)
        {
            ...
            {
                // where IsDeleted=0
                var expIsDeleted = RepositoryExtensions.EqualExpression<T, bool>("IsDeleted", false);
